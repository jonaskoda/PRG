...
// - function  ----------------------------------------------------------------
bool get_message_size(uint8_t msg_type, int *len)  //describes comunication between pc and nucleo
{
    bool ret = true;
    switch(msg_type) {
        case MSG_OK:
...
// - function  ----------------------------------------------------------------
bool parse_message_buf(const uint8_t *buf, int size, message *msg)   // nastoucha zpravu do struktury
{
