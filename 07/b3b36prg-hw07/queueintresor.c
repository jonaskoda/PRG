...
{
    int *cmpr = (*link_list)->compare;

    if (cmpr((*link_list)->first->value, temp1->value) >= 0)  // put the node behind the first one
    {
...
// temp2->next->value > temp1->value
void insert_multiple_case(linked_list_t **link_list, item *temp1, item *temp2)
{                                                              // insert the node into a queue of size
    while (temp2->next != NULL && cmpr(temp2->next->value, temp1->value) > 0) // larger than 1
    {
...
{                                                              // its value
    ...
    {  // (temp2->value <= temp1->value)
        ...
        if (cmpr(temp1->value, temp2->value) >= 0)  // possible only if temp2 is inserted as the first element
        {
            temp1->next = temp2; 
            link_list->first = temp1;
            link_list->capacity++;
...
void erase_first_case(linked_list_t **link_list, item **temp1, void *entry, int *deleted)   // (*temp1)->value == entry
{
    while((*temp1) != NULL && cmpr((*temp1)->value, entry) == 0)   // case in which the entry is present as a first
...
_Bool erase(void *queue, void *entry)                         // get rid of specified values
{
    ...
    while (temp1 != NULL)
    {         // temp1->value != entry
        while(temp1 != NULL && cmpr(temp1->value, entry) != 0)         // loop through nodes
        {
            temp2 = temp1;
            temp1 = temp1->next;
        }
